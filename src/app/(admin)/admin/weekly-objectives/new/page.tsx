...
import { Module, Chapter, ExamExercise, ProgramDuration } from "@/types/database";
...
export default function NewWeeklyObjectivePage() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const [modRes, chapRes, exRes] = await Promise.all([
      ...
      if (exRes.data) setExercises(exRes.data as ExamExercise[]);
      setLoading(false);
    }
    fetch();
  }, []);
  ...
  const handleSave = async (another = false) => {
    ...
    if (error) {
      ...
      if (another) {
        setForm({
          program_duration: form.program_duration,
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h1 className="text-2xl font-medium text-gray-900 dark:text-white">Ajouter un objectif hebdomadaire</h1>

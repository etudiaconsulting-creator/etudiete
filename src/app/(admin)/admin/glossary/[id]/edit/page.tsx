...
import { Module, GlossaryTerm } from "@/types/database";
...
export default function EditGlossaryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (term) {
          const termData = term as GlossaryTerm;
          setFormData({
            term: termData.term,
            definition: termData.definition,
            category: termData.category,
            related_terms: (termData.related_terms || []).join(", "),
            module_id: termData.module_id,
          });
        }
      } catch (err) {
        console.error("Error fetching term:", err);
        error("Erreur lors du chargement du terme");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (err) throw err;
      success("Terme mis à jour avec succès");
